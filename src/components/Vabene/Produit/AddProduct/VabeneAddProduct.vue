<template>
  <div class="card mb-25 border-0 rounded-0 bg-white add-user-card">
      ...
      <form @submit.prevent="uploadLogo">
        <div class="row">
          ...
          <div class="col-md-12" v-if="isPizzaCategory">
            ...
            <div class="mt-2">
              <div
                v-if="allPizzaIngredients.length > 0"
          ...
          <div class="col-md-12">
            <div class="form-group mb-15 mb-sm-20 mb-md-25">
              ...
              <img
                v-if="productData.image_urls.length > 0"
          ...
          <div class="col-md-6">
            <div class="form-group mb-15 mb-sm-20 mb-md-25">
              ...
              <input
                ...
                :class="{
                  'is-valid': isValidCookingTime(productData.cookingTime),
                }"
                required
              />
            </div>
          ...
          <div class="col-md-6">
            <div class="form-group mb-15 mb-sm-20 mb-md-25">
              ...
              <button
                ...
                type="button"
                data-bs-toggle="modal"
                data-bs-target="#contentModalScrollable_productSize"
              >
                {{
                  actionDetected === "edit"
                    ? "Mettre a jour variation"
            ...
            >
              <span class="position-relative">{{
                allVariationsProduct.length > 0
                  ? `${allVariationsProduct.length} taille(s)`
                  : `Aucune taille`
              }}</span>
            </span>
          </div>
    ...
    <div
      class="modal fade"
      id="contentModalScrollable_productSize"
      tabindex="-1"
      aria-hidden="true"
    >
        ...
        <div class="modal-content flex-column justify-content-center">
          <div class="modal-header">
            <h1 class="modal-title fs-5">
              Specifier les différentes taille du produit
            </h1>
            <button
              type="button"
              class="btn-close"
            ...
            <div class="flex-column">
                ...
                <div class="col-md-12">
                  <div class="form-group mb-15 mb-sm-20 mb-md-25">
                    <button
                      ...
                      :class="{
                        'opacity-50 cursor-not-allowed': !isFormValidVariation,
                      }"
                    >
                      Ajouter
                    </button>
              ...
              <div class="table-responsive">
                <table class="table text-nowrap align-middle mb-0">
                  ...
                  <tbody
                    v-if="
                      allVariationsProduct && allVariationsProduct.length > 0
                    "
                  >
                    <tr
                      v-for="(variation, index) in allVariationsProduct"
                      :key="index"
                    >
                      ...
                      <td class="shadow-none lh-1 fw-medium">
                        <span class="badge text-outline-danger"
                          >{{ variation.price }} CHF</span
                        >
                      </td>
                      <td class="shadow-none lh-1 fw-medium">
                        <span class="badge text-outline-danger"
                          >{{ variation.priceLivraison }} CHF</span
                        >
                      </td>
                      ...
                      >
                        <span
                          ...
                          >{{
                            cleanAndTruncateHtmlText(variation.description)
                          }}</span
                        >
                        <span
                          v-else
                      ...
                      <td class="shadow-none lh-1 fw-medium text-body-tertiary">
                        <button
                          @click="removeSizeVariation(variation)"
    ...
    <div
      class="modal fade"
      id="contentModalScrollable_pizzaIngredients"
      tabindex="-1"
      aria-hidden="true"
    >
      <div class="modal-dialog modal-dialog-scrollable modal-lg">
        <div class="modal-content flex-column justify-content-center">
          <div class="modal-header">
            <h1 class="modal-title fs-5">
              Sélectionner les ingrédients de base de la pizza
            </h1>
            <button
              type="button"
              class="btn-close"
          ...
          <div class="modal-body">
            <div class="flex-column">
              <div class="row">
                <div class="col-md-12">
                  <div class="form-group mb-15 mb-sm-20 mb-md-25">
                    ...
                    <div
                      v-else-if="searchIngredientQuery && !isSearching"
                      class="mt-2 text-muted"
                    >
                      <small
                        >Aucun ingrédient trouvé pour "{{
              ...
              <div class="mt-3">
                ...
                <div
                  v-if="allPizzaIngredients && allPizzaIngredients.length > 0"
              ...
              >
                <button
                  @click="validatePizzaIngredients"
                  class="btn btn-primary btn-sm"
                  type="button"
                  data-bs-dismiss="modal"
                  aria-label="Close"
...
export default defineComponent({
  ...
  methods: {
    ...
    removeSizeVariation(sizeSelectedSelected: ProductSizesModel) {
      ...
      if ((this as any).$route.params.action == ActionCrud.EDIT) {
        this.removeProductVariationProduct(
          this.productResponse?.id,
          sizeSelectedSelected.id
        );
      }
    },
    ...
    async createNewProduct() {
      ...
      const validIngredientsBaseIds = (
        this.productData.ingredientsBase || []
        ...
        if (response.code === 200) {
          if (response.data) {
            ...
            this.productData.ingredients =
              this.productResponse.Ingredients?.map((ing) => ing.id) || [];
            ...
            if (this.productResponse.longDescription) {
              this.productData.longDescription =
                this.productResponse.longDescription;
            }
            ...
            (this.productData.variationsProduct as any[]) =
              this.productResponse.productSizes;
            if (this.productResponse.productSizes.length > 0) {
              (this.allVariationsProduct as any[]) =
                this.productResponse.productSizes.map((ps) => ({
                  id: ps.id,
                  size: ps.size,
                  price: ps.price,
            ...
            if (
              this.productResponse.ingredientsBaseNames &&
              this.productResponse.ingredientsBaseNames.length > 0
            ) {
              // Charger les noms des ingrédients de base pour l'affichage
              this.allPizzaIngredients =
                this.productResponse.ingredientsBaseNames.map((name, index) => {
                  const baseIngredient =
                    this.productResponse?.ingredientsBase?.[index];
                  return {
                    id: baseIngredient?.id || `temp-${index}`,
                    name: name,
                    created_at:
                      baseIngredient?.created_at || new Date().toISOString(),
                  };
                });
              ...
              this.productData.ingredientsBase =
                this.productResponse.ingredientsBase
    ...
    async removeProductVariationProduct(productID, variationID) {
      try {
        const response = (await removeVariationProduct(
          productID,
          variationID
        )) as ApiResponse<any>;
        if (response.code === 200) {
    ...
    async fetchCategories(page = 1) {
      this.isLoading = true;
      try {
        const response = (await getAllCategories(page, 50)) as any;
        if (response.code === 200) {
          if (response.data) {
            this.originalCategories = Array.isArray(response.data)
              ? response.data
              : [];
          }
        } else {
    ...
    onCategoryChange(categoryId: string) {
      ...
      const selectedCategory = this.originalCategories.find(
    ...
    async loadAvailableIngredients() {
      try {
        ...
        console.error(
          "Erreur lors du chargement des ingrédients de base:",
          error
        );
    ...
    async searchIngredients() {
      ...
      this.searchTimeout = setTimeout(async () => {
        ...
        try {
          const response = (await listeIngredientBase(
            1,
            10,
            this.searchIngredientQuery.trim()
          )) as ApiResponse<IngredientBaseModel[]>;
          if (response.code === 200 && response.data) {
    ...
    selectIngredientFromSearch(ingredient: IngredientBaseModel) {
      ...
      const currentIngredientIds = this.allPizzaIngredients
        .map((ing) => ing.id)
    ...
    validatePizzaIngredients() {
      ...
      const allIngredientIds = [
        ...new Set([...existingIngredientIds, ...currentIngredientIds]),
      ];
    ...
    async loadExistingPizzaIngredients(ingredientIds: string[]) {
      try {
        ...
        console.error(
          "Erreur lors du chargement des ingrédients existants:",
          error
        );
  ...
  computed: {
    categoriesOptions(): CategorieModel[] {
      return Array.isArray(this.originalCategories)
        ? this.originalCategories
        : [];
    },
    typeOptions(): string[] {
      return Array.isArray(this.allTypeProduct) ? this.allTypeProduct : [];
    },
    allIngredient(): IngredientModel[] {
      const ingredients =
        this.ingredientResponse?.data?.items || this.originalIngredients;
    ...
    isPizzaCategory() {
      ...
      const selectedCategory = this.originalCategories.find(
        (cat) => cat.id === this.productData.categorieID
      );
      return (
        selectedCategory && selectedCategory.name.toLowerCase() === "pizza"
      );
    },
    isFormValid() {
      return (

<template>
  <div class="card mb-25 border-0 rounded-0 bg-white add-user-card">
        ...
        <div class="row">
          ...
          <div class="col-md-12">
            <div class="form-group mb-15 mb-sm-20 mb-md-25">
              ...
              >
                ...
                <option value="Charcuterie/viande" selected>
                  Charcuterie/viande
                </option>
                <option value="Poisson" selected>Poisson</option>
                <option value="Produit laitier" selected>
                  Produit laitier
                </option>
          ...
          <div class="col-md-6">
            <div class="form-group mb-15 mb-sm-20 mb-md-25">
              ...
              <input
                type="text"
                class="form-control shadow-none rounded-0 text-black mb-10"
                placeholder="Coller l'URL de l'image (ex: https://...)"
                v-model="ingredientData.imageUrl"
                @change="(event) => handleInput(event, 'imageUrl')"
              />
              <input
                type="file"
                class="form-control shadow-none rounded-0 text-black"
                ...
                :required="actionDetected === 'add' && !ingredientData.imageUrl"
              />
            </div>
          </div>
          <div class="col-md-6">
            <div class="form-group mb-15 mb-sm-20 mb-md-25">
              ...
              <button
                ...
                data-bs-target="#contentModalScrollable_ingredientSize"
              >
                {{
                  actionDetected === "edit"
            ...
            >
              <span class="position-relative">{{
                allVariationsIngredientSize.length > 0
                  ? `${allVariationsIngredientSize.length} taille(s)`
                  : `Aucune taille`
              }}</span>
            </span>
          </div>
    ...
    <div
      class="modal fade"
      id="contentModalScrollable_ingredientSize"
      tabindex="-1"
      aria-hidden="true"
    >
        ...
        <div class="modal-content flex-column justify-content-center">
          <div class="modal-header">
            <h1 class="modal-title fs-5">
              Specifier les différentes taille de l'ingredient
            </h1>
            <button
              type="button"
              class="btn-close"
          ...
          <div class="modal-body">
            <div class="flex-column">
              ...
              <div
                class="table-responsive flex justify-content-center items-center"
              >
                <table class="table text-nowrap align-middle mb-0">
                  ...
                  <tbody
                    v-if="
                      allVariationsIngredientSize &&
                      allVariationsIngredientSize.length > 0
                    "
                  >
                    <tr
                      v-for="(variation, index) in allVariationsIngredientSize"
                      :key="index"
                    >
              ...
              >
                <button
                  @click="updateVariationProduct(ingredientResponse?.id)"
...
export default defineComponent({
  ...
  methods: {
    addSizeVariation() {
      ...
      // Vérification unique
      const alreadyExists = this.ingredientData.ingredientSizeData.some(
    ...
    removeSizeVariation(sizeSelectedSelected: IngredientSizeModel) {
      ...
      if (index !== -1) {
        ...
        this.ingredientData.ingredientSizeData.splice(index, 1);
      }
      if ((this as any).$route.params.action == ActionCrud.EDIT) {
        this.removeProductVariationIngredient(
          this.ingredientResponse?.id,
    ...
    async updateVariationProduct(ingredientID) {
      if (this.allVariationsIngredientSize.length > 0) {
        if (this.actionDetected === "edit") {
          ...
          try {
            const response = await updateVariationIngredient(
              ingredientID,
              payload
            );
    ...
    async fetchDetailIngredient(ingredientID) {
      this.isLoading = true;
      try {
        const response = (await detailIngredient(
          ingredientID
        )) as ApiResponse<IngredientModel>;
        if (response.code === 200) {
          if (response.data) {
            this.ingredientResponse = response.data;
            const apiData = response.data as any;
            this.ingredientData.name = apiData.name;
            this.ingredientData.type = apiData.type;
            this.ingredientData.extra_cost_price = apiData.extra_cost_price;
            // Accept both camelCase and snake_case from API
            this.ingredientData.imageUrl =
              apiData.imageUrl || apiData.image_url || "";
            // Variations - accept potential snake_case as well
            const sizes =
              apiData.ingredientSizes || apiData.ingredient_sizes || [];
            this.ingredientData.ingredientSizeData = sizes || [];
            this.allVariationsIngredientSize = sizes || [];
          }
        } else {
          this.toast.error(response.message);
        }
    ...
    async uploadLogo() {
      if (this.actionDetected === "add") {
        // If user provided a direct URL, create immediately
        if (!this.logoUpload && this.ingredientData.imageUrl) {
    ...
    async removeProductVariationIngredient(ingredientID, variationID) {
      try {
        const response = (await deleteIngredientVariation(
          ingredientID,
          variationID
        )) as ApiResponse<any>;
        if (response.code === 200) {

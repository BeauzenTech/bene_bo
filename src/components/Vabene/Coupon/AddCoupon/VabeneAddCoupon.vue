    ...
    <div class="card-body p-15 p-sm-20 p-md-25 p-lg-30 letter-spacing">
      <form @submit.prevent="createNewCategorie">
        <div class="row">
          ...
          <div class="col-md-12" v-if="sendingType !== 'Fixe'">
            <div class="form-group mb-15 mb-sm-20 mb-md-25">
              ...
              <input
                ...
                :class="{
                  'is-valid': validTextField(categorieData.couponCode),
                }"
              />
            </div>
          </div>
          ...
          <div
            class="col-md-12"
            v-if="categorieResponse && actionDetected === ActionCrud.EDIT"
          >
            <div class="form-group mb-15 mb-sm-20 mb-md-25">
              <label class="d-block text-black fw-semibold mb-10">
                Activation *
              ...
              <div class="form-check form-switch">
                <input
                  ...
                  @change="
                    toggleCategorieActivation(
                      categorieResponse,
                      categorieResponse.is_active
                    )
                  "
                />
              </div>
          ...
          <div class="col-md-12 mt-30">
            <div class="form-group mb-15 mb-sm-20 mb-md-25">
              ...
              <ul class="list-unstyled" v-if="categorieResponse">
                <li
                  v-if="
                    categorieResponse.orders &&
                    categorieResponse.orders.length === 0
                  "
                >
                ...
                <li
                  v-else-if="
                    ...
                    categorieResponse.orders.length > 0
                  "
                  v-for="(order, index) in categorieResponse.orders"
                  :key="order.id"
                >
                  <span class="badge bg-light text-black fw-semibold fs-14">
                    {{ index + 1 }} - {{ order.guestEmail }}
                  </span>
                </li>
                <li v-else>
                  <span class="text-muted fst-italic"
  ...
  methods: {
    parseEligibleUsers(eligibleUsersString: string | null): string[] {
      ...
      try {
        ...
        console.error(
          "Erreur lors du parsing des utilisateurs éligibles:",
          error
        );
    ...
    async toggleCategorieActivation(categorie, status) {
      ...
      try {
        const response = (await disableCoupon(
          categorie.code,
          payload
        )) as ApiResponse<any>;
      ...
      try {
        ...
        if (response.code === 200) {
          if (response.data?.data) {
            ...
            this.originalUsers = data.filter(
              (item) => item.user != null && item.user.deviceToken != null
            );
          }
        } else {
          this.toast.error(response.message);
      ...
      try {
        ...
        if (response.code === 200) {
          if (response.data) {
            ...
            this.categorieData.type =
              this.categorieResponse.discount_type === "percent"
                ? "Pourcentage"
                : "Fixe";
            this.sendingType =
              this.categorieResponse.discount_type === "percent"
    ...
    sendingType(this: any, newVal) {
      ...
      if (newValue == "Groupé") {
        for (let i = 0; i < this.originalUsers.length; i++) {
          this.categorieData.destination.push(
            this.originalUsers[i].user.deviceToken
          );
        }
      }
      this.userSelected = null;

...
export const requestNotificationPermission = async (): Promise<string | null> => {
    try {
        ...
        if (permission === "granted") {
            ...
            console.error("❌ Permission bloquée. Change les paramètres du navigateur !");
            return null;
        }
    } catch (err) {
...
export const requestForToken = async (registration: ServiceWorkerRegistration): Promise<string | null> => {
    try {
        const token = await getToken(messaging, {
            vapidKey: "BJHn3ttRwAVIQ0iVO2ZEiDXtyZxWi5IAPFK7fBzp8-qG_CUBg0uO1XEU_-zAKjkYnbnXHTidbEam2iTL2EZsdZ0",
